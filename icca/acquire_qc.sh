#!/bin/bash
#SBATCH --job-name=acquire_qc
#SBATCH --output=acquire_qc.out
#SBATCH --error=acquire_qc.err
#SBATCH --nodes=1
#SBATCH --mem=90G
...
snakemake --unlock -s acquire_qc.py
snakemake --use-conda --cores 20 -p -s acquire_qc.py
